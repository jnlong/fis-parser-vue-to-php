    ...
    switch (node.type) {
        case 'VueExpression':
            return node.filters.reduce(function(pre, filter) {
                ...
                return ret(("$ctx->_f(\"" + (filter.name) + "\",array(" + (pre.code) + ((args.length)?(","+args.join(",")):"") + "))"));
        ...
        case 'MemberExpression':
            ...
            if (property.isStatic && property.value === "length") {
                if (obj.type === String) {
                    ...
                    return ret(("$ctx->_len(" + (obj.code) + ")"));
                }
            }
            // FIXME 这里也有可能会踩到坑
            return ret(((obj.code) + "[" + (property.code) + "]"));
        case 'Literal':
            return ret(("(" + (JSON.stringify(node.value)) + ")"), getVarType(node.value), node.value);
        case 'BinaryExpression':
            ...
            if (left.isStatic && right.isStatic) {
                ...
                return ret(JSON.stringify(value), getVarType(value), value);
            }
            if (node.operator === "+") {
                if (left.type === String || right.type === String) {
                    ...
                    return ret(("$ctx->_a(" + (left.code) + ", " + (right.code) + ")"));
                }
            }
...
function genHandler (
  ...
  if (Array.isArray(handler)) {
    return ("array(" + (handler.map(function (handler) { return genHandler(name, handler); }).join(',')) + ")")
  }
...
function bind$1 (el, dir) {
  el.wrapData = function (code) {
    return ("$ctx->_b(" + code + ",'" + (el.tag) + "'," + (dir.value) + (dir.modifiers && dir.modifiers.prop ? ',true' : '') + ")")
  };
}
...
function genElement (el) {
  if (el.staticRoot && !el.staticProcessed) {
    ...
    if (el.component) {
      ...
      code = "$ctx->_c('" + (el.tag) + "'" + (data ? ("," + data) : '') + (children ? ("," + children) : '') + ")";
    }
    // module transforms
...
function genFor (el) {
  ...
  return "$ctx->_l(" + (transformExpression(exp)) + "," +
...
function genData (el) {
  ...
  // component v-model
  if (el.model) {
    data += "\"model\"\>array(\"value\"=>" + (el.model.value) + ",\"callback\"=>" + (el.model.callback) + ",\"expression\"=>" + (el.model.expression) + "),";
  }
  // inline-template
...
function genDirectives (el) {
  ...
  for (i = 0, l = dirs.length; i < l; i++) {
    ...
    if (needRuntime) {
      hasRuntime = true;
      res += "array(\"name\"=>\"" + (dir.name) + "\",\"rawName\"=>\"" + (dir.rawName) + "\"" + (dir.value ? (",\"value\"=>(" + transformExpression(dir.value) + "),\"expression\"=>" + (varExport(dir.value))) : '') + (dir.arg ? (",\"arg\"=>\"" + (dir.arg) + "\"") : '') + (dir.modifiers ? (",\"modifiers\"=>" + (varExport(dir.modifiers))) : '') + "),";
    }
  }
  if (hasRuntime) {
    return res.slice(0, -1) + ')'
...
function genScopedSlots (slots) {
  return ("\"scopedSlots\"=>array(" + (Object.keys(slots).map(function (key) { return genScopedSlot(key, slots[key]); }).join(',')) + ")")
}
...
function genData$1 (el) {
  ...
  if (el.classBinding) {
    data += "\"class\"=>" + transformExpression(el.classBinding) + ",";
  }
  return data
}
...
function text (el, dir) {
  if (dir.value) {
    addProp(el, 'textContent', ("$ctx->_s(" + (dir.value) + ")"));
  }
}
...
function html (el, dir) {
  if (dir.value) {
    addProp(el, 'innerHTML', ("$ctx->_s(" + transformExpression(dir.value) + ")"));
  }
}
