...
Start
  = __ expression:VueExpression __ { return expression; }

VueExpression
  = head:VueExpressionNoFilter
    tail:( 
        __ "|" __ filter:VueFilter {
            return filter;
        }
    )*
    {
...
PrimaryExpression
  ...
  / ArrayLiteral
  / ObjectLiteral
  / "(" __ expression:VueExpressionNoFilter __ ")" { return expression; }
...
ElementList
  = head:(
      elision:(Elision __)? element:VueExpressionNoFilter {
        return optionalList(extractOptional(elision, 0)).concat(element);
      }
    )
    tail:(
      __ "," __ elision:(Elision __)? element:VueExpressionNoFilter {
        return optionalList(extractOptional(elision, 0)).concat(element);
      }
    )*
    { return Array.prototype.concat.apply(head, tail); }
...
//*  = key:PropertyName __ ":" __ value:AssignmentExpression {
  = key:PropertyName __ ":" __ value:VueExpressionNoFilter {
...
MemberExpression
  = head:PrimaryExpression
    tail:(
        __ "[" __ property:VueExpressionNoFilter __ "]" {
          return { property: property, computed: true };
        }
      / __ "." __ property:IdentifierName {
          return { property: property, computed: false };
...
//*
ArgumentList
  = head:VueExpressionNoFilter tail:(__ "," __ VueExpressionNoFilter)* {
      return buildList(head, tail, 3);
    }
...
MultiplicativeExpression
  = head:MemberExpression
    tail:(__ MultiplicativeOperator __ MemberExpression)*
    { return buildBinaryExpression(head, tail); }
...
ConditionalExpressionNoIn
  = test:LogicalORExpressionNoIn __
    "?" __ consequent:VueExpressionNoFilter __
    ":" __ alternate:VueExpressionNoFilter
    {
      return {
        type: "ConditionalExpression",
        test: test,
